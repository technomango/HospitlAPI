...
class ImageResize {
	...
	// application/libraries/ImageResize.php

public function resize($files, $dir_path = null, $thumb_path = null) // <-- ¡AQUÍ ESTÁ EL CAMBIO!
{
	// --- INICIO DEL BLOQUE AÑADIDO ---
// Si las rutas se pasan como argumentos, úsalas. Si no, confía en las propiedades de la clase.
if ($dir_path) {
...
// --- FIN DEL BLOQUE AÑADIDO ---
    ...
    // Asignamos las rutas recibidas a las propiedades de la clase
    $this->dir_path   = $dir_path;
    $this->thumb_path = $thumb_path;
    ...
    if ($this->file_count > 0) {
        ...
        for ($x = 0; $x < $this->file_count; $x++) {
            ...
            if ($file_mime_type != 'inode/x-empty' || $this->files['size'][$x] > 0) {
                ...
                // ----- ¡LA LÍNEA CORREGIDA! -----
                // Eliminamos la llamada a customlib y usamos la ruta correcta.
                $upload_image = $this->dir_path . basename($fileName);
                    ...
                    if ($this->create_thumb) {
                        if ($file_mime_type == 'image/jpeg' || $file_mime_type == 'image/png' || $file_mime_type == 'image/gif') {
                            ...
                            if ($imgt) {
                                ...
                                // También eliminamos la llamada a customlib aquí
                                $imgt($new_image, $this->thumb_path . $thumb_name);
                            }
                        }
                    }
                    $img_array = array(
