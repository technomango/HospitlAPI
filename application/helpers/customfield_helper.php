...
if (!function_exists('display_custom_fields')) {
    ...
    function display_custom_fields($belongs_to, $rel_id = false, $where = array()) {
        ...
        foreach ($result as $result_key => $field) {
            ...
            if ($field['type'] == 'input' || $field['type'] == 'number') {
                ...
                $type = $field['type'];
                $fields_html .= render_h3_field($field_name, $field['belong_to'], $field['id'], $field['validation'], $label, $value, $type, $input_class);
    ...
    function render_checkbox_field($name, $options, $belong_to, $field_id, $validation, $label = '', $value = '', $type = 'text', $input_class = '') {
        ...
        foreach ($options as $option_key => $option_value) {
			...
			$input .= '<div class="checkbox-list col-md-4">';
            $input .= '<label>';

            $input .= '<input type="' . $type . '" id="' . $name . '" name="' . $name . '[]"  value="' . $option_value . '" ' . set_checkbox($name, $option_value, $chk_status) . '>' . $option_value . '</label>' . '</div>';
            $input .= '</label>';
        }
	...
	function render_h3_field($name, $belong_to, $field_id, $validation, $label = '', $value = '', $type = 'h3', $input_class = '') {
        $input = '';
        $_form_group_attr = '';
        $_input_attrs = '';
        ...
        $input .= '<div class="form-group">';
        if ($label != '') {
            $input .= '<h3 class="h3-customfield" class="control-label">' . $label . '</h3>';
        }
        if ($validation) {
            $input .= "<small class='req'> *</small>";
        }
