    ...
    public function getpharmacyDatatable()
    {
        ...
        if (!empty($dt_response->data)) {
            foreach ($dt_response->data as $key => $value) {
                ...
                $action .= "<a href='#' onclick='viewDetail(" . $value->id . ")' class='btn btn-default btn-xs' data-toggle='tooltip' title='" . $this->lang->line('show') . "' ><i class='fa-regular fa-eye'></i></a>";
                if ($this->rbac->hasPrivilege('medicine_bad_stock', 'can_add')) {
                    $action .= "<a href='#' class='btn btn-default btn-xs' onclick='addbadstock(" . $value->id . ")' data-toggle='tooltip' title='" . $this->lang->line('add_bad_stock') . "' > <i class='fa-regular fa-triangle-exclamation'></i> </a>";
                }
        ...
        if (!empty($dt_response->data)) {
            foreach ($dt_response->data as $key => $value) {
                ...
                if ($this->rbac->hasPrivilege('pharmacy_partial_payment', 'can_view')) {
                    $action .= "<span class='medium-lr-tooltip'><a href='#' data-loading-text='" . $this->lang->line('please_wait') . "' data-record-id='" . $value->id . "' class='btn btn-default btn-xs add_payment tooltip-inner-2' data-toggle='tooltip' title='" . $this->lang->line('add_view_payments') . "' ><i class='fa-regular fa-file-invoice'></i></a></span>";
                }
                $action .= "<a href='#' onclick='viewDetail(" . $value->id . ")' class='btn btn-default btn-xs' data-toggle='tooltip' title='" . $this->lang->line('show') . "' ><i class='fa-regular fa-eye'></i></a>";
                $action .= "<a href='#'  data-loading-text='" . $this->lang->line('please_wait') . "' data-record-id='" . $value->id . "' class='btn btn-default btn-xs print_bill' data-toggle='tooltip' title='" . $this->lang->line('print') . "' ><i class='fa-regular fa-print'></i></a>";
                ...
                //====================
                $row[]     = $currency_symbol.number_format($value->discount) . " (" . $value->discount_percentage . "%)";
                $row[]     = $currency_symbol.number_format($value->net_amount, 2, '.', '');
                $row[]     = $currency_symbol.number_format($value->paid_amount, 2, '.', '');
                $row[]     = $currency_symbol.number_format($value->refund_amount, 2, '.', '');         
                $row[]     = $currency_symbol.number_format((float) $balance_amount, 2, '.', '');
                $dt_data[] = $row;
            }
        }
        $json_data = array(
    ...
    public function getpharmacypurchaseDatatable()
    {
        ...
        if (!empty($dt_response->data)) {
            foreach ($dt_response->data as $key => $value) {
                ...
                $action = "<div class='rowoptionview rowview-mt-19'>";
                $action = "<div class='rowoptionview rowview-mt-19'>";
                $action .= "<a href='#' onclick='viewDetail(" . $value->id . ")' class='btn btn-default btn-xs' data-toggle='tooltip' title='" . $this->lang->line('show') . "'  ><i class='fa-regular fa-reorder'></i></a>";
                if (!empty($value->file)) {
                ...
                $row[] = $value->supplier;
                $row[] = $currency_symbol.number_format($value->total, 2, '.', '');
                $row[] = $currency_symbol.number_format($value->tax, 2, '.', '');
                $row[] = $currency_symbol.number_format($value->discount, 2, '.', '');
                $row[] = $currency_symbol.number_format($value->net_amount, 2, '.', '');
                //====================
