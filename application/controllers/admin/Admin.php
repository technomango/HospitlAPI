...
class Admin extends Admin_Controller
{
    ...
    public function getpatientdatatable()
    {
        ...
        $data        = array();
        $url         = array();
        $info_data   = array('<i class="fa-regular fa-stethoscope"></i> Consulta Externa', '<i class="fa-regular fa-bed-pulse"></i> Hospitalización', '<i class="fa-regular fa-x-ray"></i> Radiología', '<i class="fa-regular fa-flask"></i> Laboratorio', '<i class="fa-regular fa-prescription-bottle-pill"></i> Farmacia', '<i class="fa-regular fa-scalpel-line-dashed"></i> Cirugía');
        $info_url    = array();
        ...
        if (!empty($dt_response->data)) {
            foreach ($dt_response->data as $key => $value) {
                ...
                $action = "<a href='#' onclick='getpatientData(" . $value->id . ")' class='pull-right'  data-toggle='modal' title='" . $this->lang->line('show') . "'><i class='fa-regular fa-eye users-iconos'></i></a>";

                $action .= "<div class='btn-group' style='margin-left:2px;'>";
                if (!empty($result[$key]['info'])) {
                    $action .= "<a href='#' style='width: 20px;border-radius: 2px;' class=''  data-toggle='dropdown' title='" . $this->lang->line('show') . "'><i class='fa-regular fa-circle-ellipsis users-iconos'></i></a>";
                    $action .= "<ul class='dropdown-menu dropdown-menu2' role='menu'>";

                    foreach ($result[$key]['info'] as $pkey => $pvalue) {
                        $action .= "<li>" . "<a href='" . $result[$key]['url'][$pkey] . "' class=''  data-toggle='' title=''>" . $pvalue . "</a>" . "</li>";
                    }
                    $action .= "</ul>";
                }
                $action .= "</div>";
    ...
    public function getdisablepatientdatatable()
    {
        ...
        if (!empty($dt_response->data)) {
            foreach ($dt_response->data as $key => $value) {
                ...
                $action = "<a href='#' onclick='getpatientData(" . $value->id . ")' class='btn btn-default btn-xs pull-right'  data-toggle='modal' title='" . $this->lang->line('show') . "'><i class='fa-regular fa-eye'></i></a>";
